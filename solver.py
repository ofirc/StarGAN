        ...
        for e in range(start, self.num_epochs):
            for i, (real_attr, real_x, real_label) in enumerate(self.data_loader):
                ...
                if (i+1) % self.d_train_repeat == 0:
                    ...
                    # the attributes always come in pairs
                    # e.g. 0:black_hair,  1:not_black_hair
                    # so div 2 will give us the attribute type.
                    #    mod 2 will give us whether it is set or unset.
                    selected_att = int(real_attr/2)

                    fake_c_dist = real_c.clone()
                    if real_attr % 2 == 1:  # attribute is unset so 0 + (+1) will set it.
                        attr_value_for_flip = real_attr % 2
                    else: # real_attr % 2 == 0  attribute is set and we want to flip it
                          #                     so 1 + (-1) will reset the attribute.
                        attr_value_for_flip = - 1

                    # face_c_dist is the attributes we want to set for
                    # the fake images (generated by the generator G).
                    # this loop goes over all the samples in the batch and
                    # flips the selected attributes.
                    # 
                    is_hair = lambda att: 0 <= att <= 2
                    hair_attrs = set([0, 1, 2])
                    for j in range(fake_c_dist.size(0)):
                        if is_hair(selected_att):
                            if attr_value_for_flip == 1:
                                # we are setting an attribute: 0 --> 1
                                # and it's a hair, we need to clear other
                                # hair attributes.
                                other_hairs = hair_attrs - set([selected_att])
                                for att in other_hairs:
                                    fake_c_dist[j][att].data -= fake_c_dist[j][att].data
                            else:
                                # we are setting an attribute: 1 --> 0
                                # and it's a hair, we need to set some
                                # other arbitrary hair attribute.
                                other_hairs = hair_attrs - set([selected_att])
                                other_hair_idx = random.choice(list(other_hairs))
                                fake_c_dist[j][other_hair_idx].data += 1
